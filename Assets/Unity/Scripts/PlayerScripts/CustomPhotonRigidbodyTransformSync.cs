...
public class CustomPhotonRigidbodyTransformSync : MonoBehaviour {
    ...
    private class State : Object
    {
        ...
        public State(Vector3 position, Vector3 velocity, Vector3 angularVelocity)
        {
        ...
        public byte[] SerializeState()
        {
            ...
            Protocol.Serialize(angularVelocity.x, buffer, ref count);
            Protocol.Serialize(angularVelocity.y, buffer, ref count);
            Protocol.Serialize(angularVelocity.z, buffer, ref count);
        ...
        public static State DeserializeState(byte[] buffer, ref int count)
        {
            ...
            Protocol.Deserialize(out state.angularVelocity.x, buffer, ref count);
            Protocol.Deserialize(out state.angularVelocity.y, buffer, ref count);
            Protocol.Deserialize(out state.angularVelocity.z, buffer, ref count);
            return state;
        }
    ...
    void FixedUpdate()
    {
        if (PhotonNetwork.isMasterClient)
            statePath.Add(new State(transform.position, rb.velocity, rb.angularVelocity));
        else if (statePath.Count > 0)
        {
            rb.velocity = statePath[0].velocity;
            rb.angularVelocity = statePath[0].angularVelocity;
